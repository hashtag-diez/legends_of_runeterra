...
contract MersenneTwister {
  ...
  function get_MT_tab() external view returns (uint, uint[624] memory) {
    uint seed = uint(
      keccak256(
        abi.encodePacked(
          block.timestamp,
...
/* 
    ...
    return (index,randValues; */
  }

  function twist(uint[624] memory MT) private pure returns (uint[624] memory) {
    for (uint i = 0; i < n; i++) {
